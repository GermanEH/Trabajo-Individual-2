...
async function getAllDogs(name) {
  try {
        ...
        cbSuccess: (response) => {
          someApiDogs = response.data.map(async (d) => {
            ...
            if (d.reference_image_id) {
              const dog = await dogs.data.find(
                (dg) => dg.image.id === d.reference_image_id
              );
              image = dog.image.url;
              ...
              if (d.temperament) {
                ...
                d.temperament = ['not temperament available '];
              } //lo hago para emparejar el tratamiento que DogCard hace con los casos de getAllDogs SIN name (que son arreglo de arreglos para poder hacer filtros combinados)
              return {
                image: image,
                name: d.name,
            ...
            return {
              //algunos de estos perros no tienen imagen (no aparecen en breeds tampoco) //también podría haber puesto image - defaultValue: null en el modelo de Dog
    ...
    if (!name) {
      ...
      return Promise.all([...apiDogs, ...localDogs]);
    }
  } catch (error) {
    console.log(error);
